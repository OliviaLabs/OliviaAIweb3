...
const FloatingMarketCapScoreBubble = ({ isOpen, onClose, title = 'Market Cap Score', score = 0, tokenName = '', addParticlesToSwarm }) => {
  const bubbleId = useState(() => `marketcap-${Date.now()}-${Math.random()}`)[0];
                ...
                <div className="flex-1 px-2 overflow-y-auto">
                  <div className="text-center space-y-1">
                    <div className="text-[7px] text-white/90 leading-tight space-y-1">
                      ...
                      <p className="text-center">Market cap score indicates the token's market position and dominance.</p>
                      <p className="text-center mt-1">
                        {score >= 70 && "Strong market position"}
                        {score >= 40 && score < 70 && "Moderate market presence"}
                        {score < 40 && "Small market cap"}
                      </p>
                    </div>
                  </div>
                </div>
...
FloatingMarketCapScoreBubble.propTypes = {
...
export default FloatingMarketCapScoreBubble;
